...
/**
 * Parser for HOCON configuration files.
 * Converts Typesafe Config to ProjectConfig model.
 */
object ConfigParser {
   ...
   * @return Either an error message or the parsed ProjectConfig
   */
   ...
   *
   * @param content The HOCON config string to parse
   * @return Either an error message or the parsed ProjectConfig
   */
  ...
  /**
   * Parse a Typesafe Config object into ProjectConfig.
   */
  private def parseConfig(config: Config): Either[String, ProjectConfig] = {
    for {
      deps <- parseDependencies(getStringList(config, "dependencies"), "dependencies")
      testDeps <- parseDependencies(getStringList(config, "testDependencies"), "testDependencies")
    } yield ProjectConfig(
   ...
   * Assumes cross-versioning by default.
   *
   * @return Either an error message or the parsed Dependency
   */
